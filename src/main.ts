...
//             paragraph("Other club participation: \n", { bold: true }),
...
//             paragraph("Ask about personal projects and the technologies the candidate used to develop these. Ask about the challenges they experienced in developing these projects and how they overcame them. This shows critical thinking and depth of understanding.\n", { italic: true }),
...
//             paragraph("Pick a question relevant to a technology that the applicant has mentioned. Make sure the applicant knows what they are talking about!\n", { italic: true }),
...
//             bullet("What are HTML, CSS, and JavaScript, and how do they work together in web development?", { bold: true }, bulletChildren(["Correct Answer:HTML provides the structure, CSS styles the appearance, and JavaScript adds interactive behavior to web pages."])),
//             bullet("Can you explain the difference between client-side and server-side programming?", { bold: true }, bulletChildren(["Correct Answer:Client-side code runs in the browser (e.g., JavaScript), while server-side code runs on the server, handling requests and - sending data to the client (e.g., Python, Node.js)."])),
//             bullet("When developing on a computer, how would you see how a website would look on different devices? (inspector)", { bold: true }, bulletChildren(["Correct Answer: Use the browser’s Developer Tools (usually F12 or right-click > Inspect), then select 'Responsive Design' mode or use the - device toolbar to preview on various screen sizes."])),
...
//             paragraph("Try to gauge how effective the applicant will be when working in a team. It can be useful to relate these questions to earlier group projects the applicant may have mentioned.\n", { italic: true }),
...
const createPages2 = async (pagesToCreate: TApplicants) => {
  ...
  for (let i = 0; i < pagesToCreate.length; i += BATCH_SIZE) {
    ...
    console.log(`Batch ${Math.floor(i / BATCH_SIZE) + 1} initial content created`);
    await sleep(2000);

    for (const response of responses) {
      try {
        const remainingBlocks: BlockObjectRequest[] = createRemainingBlocks(/* pass necessary data */);
